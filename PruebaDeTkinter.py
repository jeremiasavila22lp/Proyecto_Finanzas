...
# --- COLORES DARK MODE ---
COLOR_FONDO = "#1e1e2e"    # Azul oscuro profundo
COLOR_TARJETA = "#2a2a3d"  # Gris azulado para paneles
COLOR_TEXTO = "#ffffff"    # Blanco
COLOR_ACCENTO = "#bb86fc"  # Morado neón para botones
COLOR_TEXTO_SECUNDARIO = "#b0b0b0" # Texto gris claro
...
def ver_historial_cierres():
    ...
    for fila in datos:
        ...
        f_list = list(fila)
        f_list[1] = f"${float(f_list[1]):,.2f}"
        f_list[2] = f"${float(f_list[2]):,.2f}"
        
        # Color según estado
        tag = 'excedido' if "Excedido" in f_list[3] else 'normal'
        tabla_cierres.insert("", tk.END, values=f_list, tags=(tag,))
...
def ver_historial():
    ...
    def aplicar_filtro_tiempo(event=None):
        ...
        if categoria_seleccionada != "Todas":
            datos = [g for g in datos if str(g[2]).lower() == categoria_seleccionada.lower()]
    ...
    def filtro_busqueda():
        ...
        for i, gasto in enumerate(resultados):
            g_list = list(gasto)
            try:
                g_list[1] = f"${float(g_list[1]):.2f}"
            except: pass
            
            tag_color = 'par' if i % 2 == 0 else 'impar'
            tabla.insert("", "end", values=g_list, tags=(tag_color,)) 
    ...
    def cargar_datos_en_tabla(lista_gastos=None):
        ...
        for i, fila in enumerate(lista_gastos):
            ...
            fila_list = list(fila)
            try:
                fila_list[1] = f"${float(fila_list[1]):.2f}"
            except: pass
            tabla.insert('', 'end', values=fila_list, tags=(tag_color,))
